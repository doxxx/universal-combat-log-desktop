...
case class CombatEvent(time: Long, eventType: EventTypes.Value, actor: Entity, target: Entity, spell: String,
                       spellId: Long, amount: Int, overAmount: Int, text: String) extends LogEvent {
  def copy(newTime: Long) = CombatEvent(newTime, eventType, actor, target, spell, spellId, amount, overAmount, text)
}
