...
class LogFile(val events: List[LogEvent], val entities: Set[Entity], val spells: Set[Spell]) {
  ...
  def normalizeTimes(startTime: Long = 0): LogFile =
    new LogFile(EventProcessor.normalizeTimes(events, startTime), entities, spells)
}
